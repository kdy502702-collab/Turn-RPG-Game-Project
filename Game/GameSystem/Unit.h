...
struct ActionResult{
    int actionCode;
    std::vector<DamageResult> resultDamaged;
    Unit* executor;
};
...
class Unit
{
    ...
    public:
        ...
        bool DownMoveCount(int count){this->status.SetMoveCount(this->status.GetMoveCount()-count);
        if (this->status.GetMoveCount()>=0){
            this->status.SetMoveCount(this->status.GetCurSpeed());
            return true;
        }
        return false;
        }
        ...
        void SetIsDefense(bool isDefense){this->isDefense = isDefense;}
        ...
        int GetCurHp(){return this->status.GetCurHp();}
